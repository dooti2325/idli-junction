...
import { Plus, Trash2, Pencil, Search, ChefHat, TrendingUp, Package, Star } from 'lucide-react';
import MenuItemForm from '../../components/admin/MenuItemForm';

const DUMMY = [
  { id: '1', name: 'Regular Idli (2 Pcs)',    price: 20,  category: 'Idli',      description: 'Nagpur\'s softest steamed rice cakes.', image: '/images/idli_platter.png' },
  { id: '2', name: 'Thatte Idli',             price: 60,  category: 'Idli',      description: 'Large, disc-shaped Karnataka-style idli.', image: '/images/hero_bg.png' },
  { id: '4', name: 'Masala Dosa',             price: 60,  category: 'Dosa',      description: 'Crispy golden crepe filled with spiced potato bhaji.', image: '/images/masala_dosa.png' },
  { id: '9', name: 'Medu Vada (2 Pcs)',       price: 40,  category: 'Snacks',    description: 'Crispy savory lentil fritters.', image: '/images/medu_vada.png' },
  { id: '11', name: 'Filter Coffee',          price: 20,  category: 'Beverages', description: 'Freshly brewed South Indian decoction coffee.', image: '/images/filter_coffee.png' },
];
  ...
  useEffect(() => {
    ...
    try {
      ...
      unsub = onSnapshot(q, snap => {
        ...
        setItems(arr.length ? arr : DUMMY);
        setLoading(false);
      }, () => { setItems(DUMMY); setLoading(false); });
    } catch { setItems(DUMMY); setLoading(false); }
    return unsub;
  }, []);
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard icon={TrendingUp} label="Avg. Price (₹)" value={items.length ? Math.round(items.reduce((s, i) => s + (i.price || 0), 0) / items.length) : 0} color="bg-green-500" />
        <StatCard icon={Star}      label="Best Seller"    value="Masala Dosa"        color="bg-blue-500" />
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full">
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan="4" className="px-5 py-12 text-center font-body text-sm text-charcoal/40">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-xl overflow-hidden border border-gray-100 flex-shrink-0">
                          <img src={item.image} alt={item.name} className="w-full h-full object-cover" onError={e => { e.target.src = 'https://images.unsplash.com/photo-1589301760014-d929f39ce9b0?w=100&q=60'; }} />
                        </div>
                        <div>
